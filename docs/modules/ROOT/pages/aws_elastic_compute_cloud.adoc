...
AWS Elastic Compute Cloud provides scalable computing capacity in AWS Cloud, using Virtual machines or **instances**.
Each instance is spin up using pre-configured templates called xref:amazon_machine_image.adoc[] (AMIs).
...
Elastic Fabric Adapter provides low-latency, high-thoughput to select EC2 instance types, suitable for HPC applications
